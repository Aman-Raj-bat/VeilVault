...
[![VeilVault CI](https://github.com/ar176ranjan-sketch/VeilVault/actions/workflows/ci.yaml/badge.svg)](https://github.com/ar176ranjan-sketch/VeilVault/actions/workflows/ci.yaml)

> **Your ballot is a secret. The result is not.**

VeilVault is a privacy-first voting dApp built on [Midnight Network](https://midnight.network). Cast anonymous ballots with cryptographic guarantees — your vote choice, identity, and wallet address **never appear on-chain**. Only a verifiable tally does.

---

## 🗳️ Product Idea

VeilVault solves a fundamental problem with on-chain governance: **transparent blockchains make private voting impossible by default**. Every existing chain can reveal who voted for what. VeilVault uses Midnight's zero-knowledge proof system to implement **Private Voting** — users prove they are eligible and cast a ballot, but the choice is protected inside a ZK circuit. Only an anonymous nullifier and the aggregate tally are written to the ledger. This enables trustless, verifiable elections where the process is public but the ballots are sealed.

---

## 🔐 Privacy Model

VeilVault's privacy model is enforced at the cryptographic level — not by policy.
...
2. The voter has not voted before (nullifier uniqueness)
3. The vote choice is valid (0, 1, or 2)
4. The correct tally counter is incremented

All of this is proven without revealing any of the private inputs.
...
Copy the example and fill in your mnemonic:
...
| Test | Description |
|---|---|
| `deploys the VeilVault voting contract` | Contract deploys, ledger state initialized correctly |
| `allows a valid voter to cast a ballot privately` | ZK proof generated, tally incremented, choice hidden |
| `prevents double voting using the nullifier mechanism` | Second vote from same voter_id rejected |
...
VeilVault/
├── contracts/
│   ├── veilcontract.compact    ← Compact source (the ONLY file you write)
...
│   └── test/veilcontract.test.ts
├── scripts/wait-for-dust.ts    ← Pre-test DUST check
...
| `cast_vote` | Impure ZK | Vote choice is PRIVATE — only tally updates |
| `update_session` | Impure ZK | Admin key is PRIVATE — hash compared on-chain |
| `vault_admin_key` | Pure | Derives admin hash from secret key |
| `make_voter_nullifier` | Pure | Derives anonymous voter commitment |
...
The `vercel.json` file handles SPA routing and sets CORS headers required for WASM.
...
export ledger nullifiers: Set<Bytes<32>>; // anonymous commitments
```
