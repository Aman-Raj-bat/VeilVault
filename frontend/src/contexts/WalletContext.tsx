...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  // Poll for wallet injection — wallets inject asynchronously after page load
  useEffect(() => {
  ...
  const connect = useCallback(async (network = 'preprod') => {
    ...
    try {
      ...
      if (!wallet)
        throw new Error('No Midnight wallet detected. Please install the 1AM extension.');
      ...
      console.error('[VeilVault] Wallet connection failed:', e);
    } finally {
