...
export function createPatchedPublicDataProvider(queryUrl: string, subscriptionUrl: string) {
  ...
  async function queryLatest(query: string, address: string) {
    ...
    if (!res.ok) throw new Error(`VeilVault indexer HTTP ${res.status}`);
...
// Browser cannot use LevelDB — this keeps private state in memory only
...
// Call this after wallet.connect() — it builds all 5 required providers
